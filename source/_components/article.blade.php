...
<article class="impact-article wrapper wrapper-wide">
  ...
  <div class="impact-article__body relative lg:flex lg:align-center {{ $article->excerpt_pos }}">
    ...
    <div class="impact-article__text lg:p-16 lg:flex lg:items-center lg:absolute lg:h-full">
      <div class="bg-white lg:bg-blue lg:opacity-90 lg:text-white py-8 lg:px-[38px]">
        <h3 class="text-{{ $article->color }} lg:text-white text-xl mb-5 font-semibold">
          @if ($article->subtitle_dt)
            <span class="lg:hidden">{!! $article->subtitle !!}</span>
            <span class="hidden lg:block">{!! $article->subtitle_dt !!}</span>
          @else
            {!! $article->subtitle !!}
          @endif
        </h3>
