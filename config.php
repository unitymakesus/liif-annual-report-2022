<?php

return [
    'production' => false,
    'baseUrl' => '',
    'title' => '2022 Annual Impact Report',
    'description' => 'Redefining equity: Moving beyond what to how.',
    'social'      => [
        'linkedin' => 'https://www.linkedin.com/company/low-income-investment-fund/',
        'facebook' => 'https://www.facebook.com/liifund/',
        'twitter'  => 'https://twitter.com/Liifund',
    ],
    'collections' => [
        'leaders' => [
            'items' => [
                [
                    'name'      => 'Daniel A. Nissenbaum',
                    'name_2'    => 'Reymundo Ocañas',
                    'title'     => 'LIIF Chief Executive Officer',
                    'title_2'   => 'LIIF Board Chair Director of Community Banking, Executive Vice President, PNC Bank',
                    'slug'      => 'dear-friends',
                    'excerpt'   => 'Dear Friends, As we continue to forge through the COVID-19 pandemic, LIIF, like everyone else, has been focused on redefining our “normal.” Interestingly, it feels not far off course from what we originally charted in 2019 for this strategic plan. Halfway through this trans-formational four-year plan, we feel even more confident that had we known what we know now, our priorities would be the same: We need to center racial equity across impact-led lending, affordable housing, and early care and education, so everyone in the United States can benefit from living in a community of opportunity, equity and well-being.',
                    'image'     => '/assets/images/liif-dear-friends-letter.jpg',
                    'image_alt' => 'Daniel A. Nissenbaum and Reymundo Ocañas talking together while walking down the street.',
                    'link_url'  => 'https://www.liifund.org/justgoodcapital/2023/02/07/air-fy22-ceo-letter/',
                    'link_text' => 'Read the full letter',
                ],
                [
                    'name'      => 'Kimberly Latimer-Nelligan',
                    'title'     => 'President',
                    'slug'      => 'kim-president',
                    'excerpt'   => 'Our efforts continue to repair the long history of inequitable systems and financial institutions excluding Black, Latino and other people and communities of color. I am pleased to share with you the progress we’ve made over the past year in redefining LIIF’s role in the community development ecosystem. In 2020, we set a goal to drive $5 billion in investments over the next decade to advance racial equity. Two years in, or 20% of the way, we are 26% there, having driven approximately $1.3 billion.',
                    'image'     => '/assets/images/Kim.png',
                    'image_alt' => 'Kimberly Latimer-Nelligan',
                    'link_url'  => 'https://www.liifund.org/justgoodcapital/2023/02/07/air-fy22-president-letter/',
                    'link_text' => 'Read the full letter',
                ],
                [
                    'name'      => 'Lucy Arellano Baglieri',
                    'title'     => 'Executive Vice President and Chief Strategy Officer',
                    'slug'      => 'lucy-vp',
                    'excerpt'   => 'Halfway through this strategic plan, LIIF is reflecting not only on our work, but on the external environment. The past two years are marked by the height of the COVID-19 pandemic, highly publicized murders of Black Americans by police and extreme political turmoil. While these compounded inequities already faced by Black, Latino, and other people and communities of color, they also prompted a national reckoning on social injustice and racial inequity. As an organization committed to communities of opportunity, equity and well-being for all, in many ways this reckoning enabled our work.',
                    'image'     => '/assets/images/Lucy.png',
                    'image_alt' => 'Lucy Arellano Baglieri',
                    'link_url'  => 'https://www.liifund.org/justgoodcapital/2023/02/07/air-fy22-cso-letter/',
                    'link_text' => 'Read the full letter',
                ],
            ],
        ],
        'cards' => [
            'items' => [
                [
                    'title'     => 'Community',
                    'slug'      => 'community',
                    'color'     => 'cyan-card',
                    'image'     => '/assets/images/card-bg-community.png',
                    'image_alt' => 'Young woman helping elderly man walk down the street',
                    'prefix'    => '$',
                    'stat'      => '3.2',
                    'unit'      => 'B',
                    'desc'      => 'invested in communities',
                ],
                [
                    'title'     => 'People',
                    'slug'      => 'people',
                    'color'     => 'blue-card',
                    'image'     => '/assets/images/card-bg-people.png',
                    'image_alt' => 'Woman with name tag smiling',
                    'prefix'    => '',
                    'stat'      => '2.5',
                    'unit'      => 'M',
                    'desc'      => 'people served',
                ],
                [
                    'title'     => 'Affordable Housing',
                    'slug'      => 'affordable-housing',
                    'color'     => 'yellow-card',
                    'image'     => '/assets/images/card-bg-affordable-housing.png',
                    'image_alt' => 'Man and woman smiling as the woman cuts vegetables',
                    'prefix'    => '',
                    'stat'      => '93',
                    'unit'      => 'K',
                    'desc'      => 'homes created or preserved',
                ],
                [
                    'title'     => 'Child Care',
                    'slug'      => 'child-care',
                    'color'     => 'green-card',
                    'image'     => '/assets/images/card-bg-child-care.png',
                    'image_alt' => 'Young boy placing a toy block on top of a toy block structure',
                    'prefix'    => '',
                    'stat'      => '302',
                    'unit'      => 'K',
                    'desc'      => 'child care slots created, preserved or enhanced',
                ],
            ],
        ],
        'articles' => [
            'items' => [
                [
                    'title'         => 'Impact-Led Lending',
                    'subtitle'      => 'LIIF’s New IRP Framework Redefines Risk, Centers Equity',
                    'slug'          => 'impact-led-lending',
                    'color'   => 'blue',
                    'image'         => '/assets/images/article-bg-mom-baby.png',
                    'image_alt'     => 'Mother sitting next to father as she holds her smiling baby.',
                    'excerpt'       => 'LIIF’s strategic pillar of impact-led lending springs from our goal of nimble deployment of capital as the vehicle for effecting deeper impact in the communities we serve. With racial equity center to our mission, LIIF needed to consider where and how our efforts as a CDFI addressed the systemic inequities that create barriers to opportunity for historically excluded communities, particularly Black, Latino, and other people and communities of color. This meant moving beyond simply providing capital. We are redefining – reimagining, if you will – the impact these community investments will make for generations to come.',
                    'excerpt_pos'   => 'right',
                    'link_url'      => 'https://www.liifund.org/justgoodcapital/2023/02/07/air-fy22-strateg%e2%80%a6pact-led-lending/',
                    'link_text'     => 'Read more',
                ],
                [
                    'title'         => 'Affordable Housing',
                    'subtitle'      => 'Solid Partnerships and a Foundation of LIIF\'s Work',
                    'slug'          => 'affordable-housing',
                    'color'   => 'blue',
                    'image'         => '/assets/images/article-bg-mom-teach.png',
                    'image_alt'     => 'Mother helping her son with homework.',
                    'excerpt'       => 'LIIF has a longstanding history of innovative capital deployment that supports the expansion of the housing supply. While we have invested over $xx that\s produced over xx units, we continued to grapple with a persistent housing crisis that significantly burdened Black, Latino and other communities of color. As we embarked on our strategic planning journey, we recognized producing units was not enouch. We needed a strategy that addressed the systemmic racism that encouraged the dismantling of Black and Latino communities. In order to make true impact we needed a to create and leverage strategic partnerships.',
                    'excerpt_pos'   => 'left',
                    'link_url'      => 'https://www.liifund.org/justgoodcapital/2023/02/07/air-fy22-strateg%e2%80%a6fordable-housing/',
                    'link_text'     => 'Read more',
                ],
                [
                    'title'         => 'Early Care and Education',
                    'subtitle'      => 'LIIF Supporting the Sector to Set Up Parents and Children for Success',
                    'slug'          => 'children-success',
                    'color'   => 'blue',
                    'image'         => '/assets/images/article-bg-mom-teach.png',
                    'image_alt'     => 'Mother helping her son with homework.',
                    'excerpt'       => 'Creating communities of opportunity, equity and well-being requires us to examine how historically excluded communities are able to access quality, affordable child care. Through our strategic planning process, LIIF identified early child care and education (ECE) as a vital component to a thriving community because when children are provided with high-quality care and education, they are able to succeed. When providers are given adequate funding, they are able to create nurturing learning environments for the children in their communities. When parents are able to access reliable, quality care they can afford, they are able to provide for their families. Read more',
                    'excerpt_pos'   => 'left',
                    'link_url'      => 'https://www.liifund.org/justgoodcapital/2023/02/07/air-fy22-strateg%e2%80%a6re-and-education/',
                    'link_text'     => 'Read more',
                ],
            ],
        ],
        'supporters' => [
            'items' => [
                [
                    'label' => 'Funders & Donors',
                    'slug' => 'funders-donors',
                    'items' => [
                        'Alphadyne Foundation',
                        'Ballmer Group',
                        'Bank of America Charitable Foundation',
                        'Bank of Hope',
                        'BBVA Foundation',
                        'Cathay Bank Foundation',
                        'Charles Schwab Bank',
                        'Chicago Community Foundation',
                        'CIT Bank, N.A.',
                        'City and County of San Francisco',
                        'City National Bank',
                        'City of Los Angeles',
                        'DC Office of the State Superintendent of Education',
                        'Early Childhood Partners Fund in The New York Community Trust',
                        'Esther A. and Joseph Klingenstein Fund',
                        'Federal Reserve Bank of San Francisco',
                        'First 5 Contra Costa',
                        'First 5 LA',
                        'First 5 San Luis Obispo County',
                        'Goldman Sachs',
                        'Heising-Simons Foundation ',
                        'Hellman Foundation ',
                        'Local Initiatives Support Corporation (LISC)',
                        'Morgan Stanley',
                        'MUFG Union Bank, N.A.',
                        'Purpose Built Communities',
                        'Robert Wood Johnson Foundation',
                        'Robin Hood',
                        'Silicon Valley Community Foundation donor advised funds',
                        'The Capital One Fund (a fund of Tides Foundation)',
                        'The JPB Foundation',
                        'The Kresge Foundation',
                        'Thurstan R. Bannister',
                        'Tipping Point Community',
                        'U.S. Bank Foundation',
                        'U.S. Department of the Treasury CDFI Fund',
                        'Wells Fargo Bank, N.A.',
                    ],
                ],
                [
                    'label' => 'Investors',
                    'slug' => 'investors',
                    'items' => [
                        'Amalgamated Bank',
                        'Banc of America Community Development Corporation',
                        'BBVA USA',
                        'Capital One',
                        'Charles Schwab Bank',
                        'CIT Bank',
                        'Community Foundation for Greater Atlanta',
                        'Deutsche Bank Trust Company Americas',
                        'Erich & Hannah Sachs Foundation',
                        'Federal Home Loan Bank of San Francisco',
                        'First Republic Bank ',
                        'Ford Foundation',
                        'HSBC Bank USA, N.A.',
                        'JPMorgan Chase',
                        'Mercy Investment Services, Inc. ',
                        'Metropolitan Transportation Commission',
                        'Mizuho Corporate Bank',
                        'Morgan Stanley',
                        'Northern Trust',
                        'New York Quarterly Meeting of the Religious Society of Friends',
                        'San Francisco Friends School',
                        'Santander Bank',
                        'Signature Bank',
                        'Sisters of Charity of the Incarnate Word',
                        'Trinity Health Corporation',
                        'U.S. Bank National Association',
                        'U.S. Department of the Treasury Community Development Financial Institutions Fund',
                        'U.S. Department of the Treasury Small Business Lending Fund',
                        'Wells Fargo',
                    ],
                ],
                [
                    'label' => 'Financing Partners',
                    'slug' => 'financing-partners',
                    'items' => [
                        'A Regional Coalition for Housing (ARCH)',
                        'Amalgamated Bank',
                        'Ballmer Group',
                        'California Community Foundation',
                        'California Department of Housing & Community Development',
                        'Calvert Impact Capital',
                        'Calvert Social Investment Foundation',
                        'Capital Impact Partners',
                        'Century Housing Corporation',
                        'City First Bank',
                        'City of NY Department of Housing Preservation and Development',
                        'Community Foundation of Greater Atlanta',
                        'Community Loan Fund of New Jersey',
                        'Community Vision',
                        'Corporation for Supportive Housing (CSH)',
                        'District of Columbia Department of Housing and Community Development',
                        'Enterprise Community Loan Fund',
                        'Enterprise Community Partners',
                        'Evelyn and Walter Haas, Jr. Fund',
                        'Great Communities Collaborative',
                        'Housing Trust Silicon Valley (HTSV)',
                        'Invest Atlanta',
                        'JPMorgan Chase',
                        'King County Housing Authority',
                        'Living Cities Blended Catalyst Fund',
                        'Los Angeles County Metropolitan Transporation Authority (MTA)',
                        'Local Initiatives Support Corporation (LISC)',
                        'Los Angeles County Development Authority, Community Development Commission',
                        'LOCUS Impact Investing',
                        'Mercy Loan Fund',
                        'Meta Housing Corporation',
                        'Metropolitan Transportation Commission',
                        'Mimi and Peter Haas Fund',
                        'Morgan Stanley',
                        'MUFG Bank',
                        'Natural Resources Defense Council',
                        'New York City Acquisition Fund',
                        'Nonprofit Finance Fund',
                        'Northern California Community Loan Fund',
                        'Purpose Built Communities (via grant from the Truist Foundation)',
                        'redefinED atlanta',
                        'Robert Wood Johnson Foundation',
                        'Rural Community Assistance Corporation (RCAC)',
                        'San Francisco Office of Early Care and Education ',
                        'Seattle Office of Housing',
                        'Self-Help Credit Union',
                        'State Charter Schools Foundation of Georgia',
                        'Tennessee Charter School Center ',
                        'The California Endowment',
                        'The JPB Foundation',
                        'The Kresge Foundation',
                        'The Prudential Insurance Company of America',
                        'The Reinvestment Fund',
                        'UMC Benefit Board, Inc.',
                        'U.S. Bank',
                        'U.S. Department of Education',
                        'Weingart Foundation',
                    ],
                ],
                [
                    'label' => 'Board of Directors',
                    'slug' => 'board-of-directors',
                    'items' => [
                        [
                            'name' => 'Reymundo Ocañas, Chair',
                            'title' => 'Director of Community Development Banking, PNC Bank',
                        ],
                        [
                            'name' => 'Russell Bruemmer, Secretary',
                            'title' => 'Retired Partner, Wilmer Cutler, Pickering, Hale and Dorr',
                        ],
                        [
                            'name' => 'Carol Naughton, Treasurer',
                            'title' => 'CEO, Purpose Built Communities',
                        ],
                        [
                            'name' => 'Margaret Anadu',
                            'title' => 'Global Head of Sustainability and Impact for Asset Management, Goldman Sachs',
                        ],
                        [
                            'name' => 'Derek Douglas',
                            'title' => 'Vice President for Civic Engagement and External Affairs, University of Chicago',
                        ],
                        [
                            'name' => 'David Fleming, MD',
                            'title' => 'Distinguished Fellow, Trust for America’s Health',
                        ],
                        [
                            'name' => 'Donna Gambrell',
                            'title' => 'President & CEO, Appalachian Community Capital;  Former Director, CDFI Fund',
                        ],
                        [
                            'name' => 'Calvin Gladney',
                            'title' => 'President & CEO, Smart Growth America',
                        ],
                        [
                            'name' => 'William Kelly, Jr.',
                            'title' => 'Founding President, Stewards of Affordable Housing for the Future',
                        ],
                        [
                            'name' => 'Dionne Nelson',
                            'title' => 'President & CEO, Laurel Street',
                        ],
                        [
                            'name' => 'Daniel A. Nissenbaum',
                            'title' => 'CEO, Low Income Investment Fund',
                        ],
                        [
                            'name' => 'Michael A. Solomon',
                            'title' => 'Managing Director of Community Development, Charles Schwab Bank & Trust Services',
                        ],
                    ],
                ],
                [
                    'label' => 'Committees',
                    'slug' => 'committees',
                    'items' => [
                        [
                            'name' => 'Lending Risk Committee',
                            'items' => [
                                [
                                    'name' => 'William Kelly, Jr. (Chair)',
                                    'title' => 'Stewards of Affordable Housing for the Future',
                                ],
                                [
                                    'name' => 'Russell Bruemmer',
                                    'title' => '(formerly) Wilmer Cutler, Pickering, Hale and Dorr',
                                ],
                                [
                                    'name' => 'Donna Gambrell',
                                    'title' => 'Appalachian Community Capital',
                                ],
                                [
                                    'name' => 'Calvin Gladney',
                                    'title' => 'Smart Growth America',
                                ],
                                [
                                    'name' => 'Michael A. Solomon',
                                    'title' => 'Charles Schwab Bank & Trust Services',
                                ],
                                [
                                    'name' => 'Daniel A. Nissenbaum',
                                    'title' => 'LIIF (alternate)',
                                ],
                            ],
                        ],
                        [
                            'name' => 'New Markets Tax Credit Advisory Board',
                            'items' => [
                                [
                                    'name' => 'Mikki Anderson',
                                    'title' => 'Purpose Built Communities',
                                ],
                                [
                                    'name' => 'Althea Arnold',
                                    'title' => 'Stewards of Affordable Housing for the Future',
                                ],
                                [
                                    'name' => 'Marla Dean',
                                    'title' => 'Bright Beginnings',
                                ],
                                [
                                    'name' => 'Ted Lempert',
                                    'title' => 'Children Now',
                                ],
                                [
                                    'name' => 'Kimberly McKay',
                                    'title' => 'BRIDGE Housing',
                                ],
                                [
                                    'name' => 'Thomas Yu',
                                    'title' => 'Asian Americans for Equality',
                                ],
                            ],
                        ],
                        [
                            'name' => 'Eastern Region Advisory Committee',
                            'items' => [
                                [
                                    'name' => 'David Beer',
                                    'title' => 'Breaking Ground',
                                ],
                                [
                                    'name' => 'Amy Brusiloff',
                                    'title' => 'Bank of America',
                                ],
                                [
                                    'name' => 'Becky Koch',
                                    'title' => 'HSBC Bank USA, N.A.',
                                ],
                                [
                                    'name' => 'Lesley Palmer',
                                    'title' => 'Mizuho Bank USA',
                                ],
                                [
                                    'name' => 'Mariadele Priest',
                                    'title' => 'Capital One',
                                ],
                                [
                                    'name' => 'Rebecca Regan',
                                    'title' => 'Terra Search Partners',
                                ],
                                [
                                    'name' => 'Kristen Scheyder',
                                    'title' => 'Citi Foundation',
                                ],
                                [
                                    'name' => 'Joan Tally',
                                    'title' => 'Morgan Stanley',
                                ],
                                [
                                    'name' => 'David Umansky',
                                    'title' => 'Civic Builders',
                                ],
                            ],
                        ],
                        [
                            'name' => 'Western Region Advisory Committee',
                            'items' => [
                                [
                                    'name' => 'Allison Brooks',
                                    'title' => 'Bay Area Regional Collaborative',
                                ],
                                [
                                    'name' => 'Maria Bustria-Glickman',
                                    'title' => 'U.S. Bank Community Development Corporation',
                                ],
                                [
                                    'name' => 'David Erickson',
                                    'title' => 'Federal Reserve Bank of New York',
                                ],
                                [
                                    'name' => 'Tiena Johnson-Hall',
                                    'title' => 'BBVA Compass',
                                ],
                                [
                                    'name' => 'Laura Kozel',
                                    'title' => 'Rocketship Education',
                                ],
                                [
                                    'name' => 'Suny Lay Chang',
                                    'title' => 'LINC Housing',
                                ],
                                [
                                    'name' => 'Richard Mandel',
                                    'title' => 'California Housing Partnership Corporation',
                                ],
                                [
                                    'name' => 'Adam Miller',
                                    'title' => 'Swing Left',
                                ],
                                [
                                    'name' => 'Arjun Nagarkatti',
                                    'title' => 'AMCAL Housing',
                                ],
                                [
                                    'name' => 'Debbie Ruane',
                                    'title' => 'Norwood Development Strategies',
                                ],
                                [
                                    'name' => 'Doug Shoemaker',
                                    'title' => 'Mercy Housing California',
                                ],
                                [
                                    'name' => 'Tim Silva',
                                    'title' => 'Wells Fargo',
                                ],
                            ],
                        ],
                        [
                            'name' => 'San Francisco Child Care Facilities Fund Program Advisory Committee',
                            'items' => [
                                [
                                    'name' => 'Fonda Davidson',
                                    'title' => 'Cross Cultural Family Center',
                                ],
                                [
                                    'name' => 'Maria del Rosario Villasana',
                                    'title' => 'City College of San Francisco',
                                ],
                                [
                                    'name' => 'Melinda Deng',
                                    'title' => 'Kai Ming',
                                ],
                                [
                                    'name' => 'Graham Dobson',
                                    'title' => 'San Francisco Office of Early Care and Education',
                                ],
                                [
                                    'name' => 'Dannhae Herrera Wilson',
                                    'title' => 'Mission Economic Development Agency',
                                ],
                                [
                                    'name' => 'Anne Kwong',
                                    'title' => 'Chinatown Community Children’s Center',
                                ],
                                [
                                    'name' => 'Lawland Long',
                                    'title' => 'FACES SF',
                                ],
                                [
                                    'name' => 'Mona Malan',
                                    'title' => 'Children’s Council of San Francisco',
                                ],
                                [
                                    'name' => 'Christina Marchiel',
                                    'title' => 'Mission Kids Co-Op',
                                ],
                                [
                                    'name' => 'Michael Neumann',
                                    'title' => 'Wu Yee Children’s Services',
                                ],
                                [
                                    'name' => 'Mario Paz',
                                    'title' => 'Good Samaritan Family Resource Center, Inc.',
                                ],
                                [
                                    'name' => 'Corey Santillan',
                                    'title' => 'La Bamba Preschool Family Child Care Home',
                                ],
                                [
                                    'name' => 'Yohanna Quiroz',
                                    'title' => 'Felton Institute',
                                ],
                                [
                                    'name' => 'Oscar Tang',
                                    'title' => 'Family Child Care Assocation of SF & CA Family Child Care Network',
                                ],
                                [
                                    'name' => 'Mark Tao',
                                    'title' => 'Wu Yee Children’s Services',
                                ],
                            ],
                        ],
                    ],
                ],
                [
                    'label' => 'Senior Staff',
                    'slug' => 'senior-staff',
                    'items' => [
                        [
                            'name' => 'Sumit Advani',
                            'title' => 'Director of Treasury',
                        ],
                        [
                            'name' => 'Amir Ali',
                            'title' => 'Deputy Director, Southeast',
                        ],
                        [
                            'name' => 'Lucy Arellano Baglieri',
                            'title' => 'Chief Strategy Officer & EVP',
                        ],
                        [
                            'name' => 'Javier Argudo',
                            'title' => 'Director, Financial Planning & Analysis',
                        ],
                        [
                            'name' => 'Sabrina Baptiste',
                            'title' => 'Chief Administrative Officer & EVP',
                        ],
                        [
                            'name' => 'Rachel Bluestein',
                            'title' => 'SVP, National Programs',
                        ],
                        [
                            'name' => 'Jo Bringazi',
                            'title' => 'Director of IT',
                        ],
                        [
                            'name' => 'Emma Chávez',
                            'title' => 'Market Director, Western Region',
                        ],
                        [
                            'name' => 'Kim DiGiacomo',
                            'title' => 'National Program Director, Early Care and Education',
                        ],
                        [
                            'name' => 'Art Fatum',
                            'title' => 'Interim Chief Financial Officer',
                        ],
                        [
                            'name' => 'Sondra Ford',
                            'title' => 'Director, SPARCC Initiative',
                        ],
                        [
                            'name' => 'Eliisa Frazier',
                            'title' => 'Director, Racial Equity & Impact Lending',
                        ],
                        [
                            'name' => 'Angie Garling',
                            'title' => 'Vice President, Early Care & Education',
                        ],
                        [
                            'name' => 'Samantha Hojo',
                            'title' => 'Director, Strategic Communications & Influence',
                        ],
                        [
                            'name' => 'Roxanne Huey',
                            'title' => 'Vice President, Finance & Corporate Controller',
                        ],
                        [
                            'name' => 'Charlie Hutton',
                            'title' => 'Director, Portfolio Accounting',
                        ],
                        [
                            'name' => 'Susan Hyman',
                            'title' => 'Chief Credit Officer & SVP',
                        ],
                        [
                            'name' => 'Alexis Iwanisziw',
                            'title' => 'Chief of Staff',
                        ],
                        [
                            'name' => 'Laura Jackman',
                            'title' => 'Deputy Director, Early Care & Education',
                        ],
                        [
                            'name' => 'Celeste Krishna',
                            'title' => 'Director, Loan Administration & Asset Management',
                        ],
                        [
                            'name' => 'LaToya Kyle',
                            'title' => 'Market Director, Southeast Region',
                        ],
                        [
                            'name' => 'Kimberly Latimer-Nelligan',
                            'title' => 'President',
                        ],
                        [
                            'name' => 'Ivan Levi',
                            'title' => 'Credit Officer & Director',
                        ],
                        [
                            'name' => 'Stephanie McFadden',
                            'title' => 'Chief Lending Officer',
                        ],
                        [
                            'name' => 'Kevin McVey',
                            'title' => 'Deputy Director, New Markets Tax Credits',
                        ],
                        [
                            'name' => 'Mario Moreno',
                            'title' => 'Director, Grants Management & Operations',
                        ],
                        [
                            'name' => 'Maygen Moore',
                            'title' => 'Director, National Lending Initiatives',
                        ],
                        [
                            'name' => 'Daniel A. Nissenbaum',
                            'title' => 'CEO',
                        ],
                        [
                            'name' => 'Galina Parastaeva',
                            'title' => 'Director of Compliance',
                        ],
                        [
                            'name' => 'Kenny Rodgers',
                            'title' => 'Deputy Market Director, Coachella Valley',
                        ],
                        [
                            'name' => 'Kirsten Shaw',
                            'title' => 'Vice President, Northeast & Mid-Atlantic Regions',
                        ],
                        [
                            'name' => 'Surat Sinha',
                            'title' => 'Director of Employee Services',
                        ],
                        [
                            'name' => 'Carolyn Smith',
                            'title' => 'Vice President, Strategic Communications',
                        ],
                        [
                            'name' => 'Jessica Standiford',
                            'title' => 'Vice President, Development & Impact Investing',
                        ],
                        [
                            'name' => 'Ken Stewart',
                            'title' => 'Deputy Director, Mid-Atlantic Region',
                        ],
                        [
                            'name' => 'Paul Wetterholm',
                            'title' => 'Assistant Controller',
                        ],
                        [
                            'name' => 'Victoria Whittaker',
                            'title' => 'Director, National Programs',
                        ],
                    ],
                ],
            ],
        ],
        'financials' => [
            'items' => [
                [
                    'label' => 'Financial Position',
                    'slug' => 'financial-position',
                    'items' => [],
                ],
                [
                    'label' => 'Financial Activities',
                    'slug' => 'financial-activities',
                    'items' => [],
                ],
            ],
        ],
    ],
];
